...
```{r}
mlogit_model_U <- mlogit(vote ~ 0 | age + sex + income | 0, 
                  data = idxData, reflevel = "U")

summary(mlogit_model_U)
```
...
```{r}
mlogit_model_no_A <- mlogit(vote ~ 0 | age + sex + income | 0, 
                  data = idxData, reflevel = "U", alt.subset = c("Y", "N", "U"))

summary(mlogit_model_no_A)
```
...
hmftest(mlogit_model_U, mlogit_model_no_A)
