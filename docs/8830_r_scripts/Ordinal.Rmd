...
Notice, the output displays two intercepts - these are out two cutpoints or $\tau$. We will have as many cutpoints as one less than our number of DV categories ($m-1$) .
...
```{r}
ctable <- cbind(ctable, "p value" = round(p_values, 5))
ctable
```
...
```{r}
world_values_gologit <- vglm(poverty ~ gender + degree  + age + country + religion,
                             family = cumulative(parallel = ~ age + degree + gender+ country),
                             data = world_values)
