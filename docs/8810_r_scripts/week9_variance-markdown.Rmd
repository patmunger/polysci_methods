...
See how the residuals are scattered around $0$ with no discernable pattern accross the range of fitted values. This is homoskedasticity. 
...
```{r}
cov_m1 <- vcovHC(my_model, mclass(rob_m1ethod = "HC0"))
rob_m1 <- sqrt(diag(cov_m1))
```

In stargazer we include the line `se = (list(rob_m1))`.
```{r, warning = FALSE}
stargazer(my_model, 
...
Let's compare the base model with the WLS, Robust, and Clustered SE models to see how standard errors (and thus p-values) are affected. We'll report the actual p-values rather than significance stars. 
