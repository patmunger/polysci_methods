...
typeof() # provides the internal storage type of the object (eg. double, list, haracter, etc.)
...
Now let's make a bar plot. A bar plot rewuires a non-numeric (non-continuous) variable. Let's use the `regime_type` variable which is class integer with four possible values.
...
Now let's do a histogram. This is similar to a barplot but we can use a continuou, numeric variable, as the values will be aggregated into bins. Let's use the `democracy` variable.
